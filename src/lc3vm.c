...
void rti(uint16_t i)
{
  ...
  {
    reg[SSP] = reg[R6]; // SSP = R6
    reg[R6] = reg[USP]; // R6 = USP
  }
}
...
void trap(uint16_t i)
{
  ...
  reg[RPC] = mem_read(TRP(i)); // PC = mem[ZEXT(trapvect8)]
}
...
{
  reg[PSR] &= 0xF8FF;         // zero out bits [10:8] bits first
  reg[PSR] |= (p & 0x7) << 8; // then OR in the new priority
}
